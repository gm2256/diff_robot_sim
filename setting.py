...
def stop_container(sim,container):
    
    sim.setBufferProperty(container, 'customData.__ctrl__', sim.packTable({'vel': 0.0}))    
    

def activate_container(sim,container):

    sim.setBufferProperty(container, 'customData.__ctrl__', sim.packTable({'vel': 0.05}))
